...
class Feature:
    ...
    def _velocity_minimum(
        ...
        # (only available for GPR models)
        if isinstance(self.model, GaussianProcessModel):
            # For some reason, sample() outputs shape (N_wave, N_samples=100)
            samples = self.model.sample(wave, n_samples)
            ...
            # For spline models, use a simple wavelength resolution estimate
            lam_min_err = float(np.diff(wave).mean())
    ...
    def _velocity_blue_edge(
        ...
        **kwargs,
        ...
        if profile_params is None:
            mu = float(wave[flux.argmin()])
            sigma = float(0.5 * (wave[-1] - wave[0]))
            profile_params = (mu, sigma, -0.1, 0.5)
            bounds = (
                (float(wave[0]), 0.0, -2.0, 0.0),
                (float(wave[-1]), 5.0 * sigma, 0.0, 1.0),
            )
        ...
        # Calculate lambda error through sampling (only for GPR models)
        if isinstance(self.model, GaussianProcessModel):
            samples = self.model.sample(wave, n_samples).T
            ...
            # For spline models, use a covariance estimate based on sigma
            lam_err = float(0.1 * sigma)  # Simple heuristic
    ...
    def pEW(
        ...
        # Calculate error from sampling (only for GPR models)
        if isinstance(self.model, GaussianProcessModel):
            # For some reason, sample() outputs shape (N_wave, N_samples=100)
            samples = self.model.sample(wave, n_samples).T
            ...
            # For spline models, use a simple percentage estimate
            pEW_err = 0.05 * abs(pEW)  # 5% uncertainty heuristic
